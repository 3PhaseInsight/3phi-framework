...
from threephi_framework.dtu.electric_heating_identifier import feature_extraction, hierarchical_clustering, label_propagation, logistic_regression
...
class ElectricHeatingIdentifier(BaseDataApp):
    ...
    def _build_dtu_cfg(self) -> dict:
        ...
        cfg.setdefault("results_dir", f"electric_heating_identifier")
    ...
    def identify_EH_applicances(self) -> None:
        ...
        # Generate list of smart meters based on config. If either sm_ids or unit_ids is 'all', then process all smart meters with data
        ...
        logging.info(f"Identifying electric heating appliances for {len(self.sm_ids)} smart meters remaining after filtering.")
        ...
        if self.sm_ids:
            ...
            for sm_id in self.sm_ids:
                for phase_str in ["L1", "L2", "L3"]:
                    meta_results = self.meta_controller.query_run_results(source="Electric Heating Identifier", meter_id=int(sm_id), phase=phase_str, label_type="Feature Engineering")
                    if meta_results:
            ...
            feature_results = pd.DataFrame(scaled_features_np, index=feature_results.index, columns=feature_results.columns)
...
if __name__ == "__main__":
    config = {
        ...
        "unit_ids": None,
        "sm_ids": ['23405', '121184', '197256', '382729', '440937', '445107', '566340', '594794', '759234', '790516', '835841', '978146'],
        "save_plots": False,
