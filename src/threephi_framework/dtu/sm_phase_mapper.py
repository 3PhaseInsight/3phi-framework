...
def _check_window_for_corrupted_data(start_end_tuple, data, lower_v_lim, v_step_lim) -> bool:
    ...
    if (differenced_abs_window > v_step_lim).any():
        return True

    return False

def _clean_data_of_single_unit(trafo_data_df,
                                unconnected_phases,
                                window_size: int = 30,
    ...
    # Save a copy of the raw data in case that another cleaning iteration with different max_corruption_threshold is required
    trafo_data_df_raw = trafo_data_df.copy()

    n_min = 7500

    unconnected_phases = [phases.replace("_l", "_l".upper()) for phases in unconnected_phases]

    # Repeats cleaning with decreasing tolerance for corrupted sm phases (max_corruption_threshold) until cleaned data is sufficiently large
    while True:

        # Drop unconnected phases
        trafo_data_df = trafo_data_df.drop(columns=[phase for phase in unconnected_phases if phase in trafo_data_df.columns])
        ...
        if nan_phases:
            logging.info(f"[Transformer {trafo_id}] The following phases have more than {max_corruption_threshold * 100}% NaNs and are excluded from clustering: {', '.join(str(u[8:]) for u in nan_phases)}")
            trafo_data_df = trafo_data_df.drop(columns=nan_phases)

        # Remove all columns which contain mostly values below 207
        constant_offset_phases = trafo_data_df.columns[(trafo_data_df < lower_v_lim).mean() > max_corruption_threshold].tolist()

        if constant_offset_phases:
            logging.info(f"[Transformer {trafo_id}] The following phases have more than {max_corruption_threshold * 100}% below {lower_v_lim}V and are excluded from clustering: {', '.join(str(u[8:]) for u in constant_offset_phases)}")
            trafo_data_df = trafo_data_df.drop(columns=constant_offset_phases)

        if trafo_data_df.shape[1] == 0:
            raise RuntimeError(f"[Transformer {trafo_id}] Remaining dataset after removing corrupted SM voltage phases is empty.")
        ...
        for start, end in [(i, i + window_size) for i in range(len(unit_data_np) - window_size + 1)]:
            is_corrupted = _check_window_for_corrupted_data(start_end_tuple=(start, end),
                                                                    data=unit_data_np,
                                                                    lower_v_lim=lower_v_lim,
                                                                    v_step_lim=v_step_lim)
        ...
        if not len(trafo_data_df.dropna()) < n_min:
            ...
            logging.info(f"[Transformer {trafo_id}] Cleaned data too little. Repeating with more conservative max_corruption_threshold {max_corruption_threshold}.")
...
def _preprocess_data_of_single_unit(trafo_data_df_clean) -> pd.DataFrame:
    ...
    trafo_data_df_clean = pd.DataFrame(scaler.fit_transform(trafo_data_df_clean), columns=trafo_data_df_clean.columns)
...
def _add_cabinet_and_feeder_id_to_column_name(trafo_data_df, SM_topology_mapping):
    ...
    for col in trafo_data_df.columns:
        ...
        if len(parts) != 3 or not col.startswith("voltage_"):
            raise ValueError(
                f"Unexpected voltage column format: {col}. "
                "Expected format: voltage_l1_<meter_id>"
            )
        ...
        if row.empty:
            raise ValueError(
                f"No topology mapping found for meter_id={sm_id}. "
                f"Column={col}."
            )
...
def _hierarchical_clustering(trafo_data_df):
    ...
    # The first two entries of each row provide the cluster indexes (not row index) of the two clusters which were merged to form the cluster of this row
...
def _check_majority_feeder(sm_phases_in_cluster, majority_feeder_threshold=0.5):
    ...
    majority_feeder = feeder_ids_counts.idxmax() if majority_feeder_count >= majority_feeder_threshold * len(sm_phases_in_cluster) else None
...
def _get_all_sm_phases_in_cluster(linkage_matrix, cluster_idx, trafo_data_df):  # idx is the indices of the cluster which results from merging two clusters below

    if cluster_idx < len(trafo_data_df.columns):  # It's a leaf (individual column) -> then there is no cluster below anymore that we could break down
        return [trafo_data_df.columns[cluster_idx]]  # The cluster indexes start with the individual column clusters 0, 1, ... len(df.columns)
    else:
        # Recursive retrieval for merged clusters
        left_idx = int(linkage_matrix[cluster_idx - len(trafo_data_df.columns), 0])  # The indexes of the two merged clusters are stored in the row of the parent cluster
        right_idx = int(linkage_matrix[cluster_idx - len(trafo_data_df.columns), 1])  # However, cluster_idx != row_idx since the individual col clusters are not in Z, so they need to be subtracted
        ...
        # So we are not returning anything until we are on the individual col cluster level, and here all the individual function results are summed
        return _get_all_sm_phases_in_cluster(linkage_matrix, left_idx, trafo_data_df) + _get_all_sm_phases_in_cluster(linkage_matrix, right_idx, trafo_data_df)
...
def _create_backup_phase_reference(trafo_data_df_cols, trafo_phase_clusters):
    ...
    trafo_phase_clusters_new_per_feeder = {"l1": [],
                                           "l2": [],
                                           "l3": []}
    ...
    if any(not value_list for value_list in trafo_phase_clusters_new_per_feeder.values()):
        raise ValueError("One or more phase clusters are empty in backup phase reference. Check if trafo have low number of feeders or if there is a very dominant feeder phase majority in the trafo which leads to all phases being assigned to one trafo phase cluster and thus no clear phase majority on feeder level.")
...
def _determine_trafo_clusters(trafo_data_df_cols, linkage_matrix):
    ...
    for col, label in zip(trafo_data_df_cols, sm_phase_to_trafo_phase_allocation):
        trafo_phase_clusters[label].append(col)

    trafo_phase_clusters = {_check_majority_phase(cols, majority_phase_threshold=0.5): cols for cols in trafo_phase_clusters.values()}

    return trafo_phase_clusters
...
def _feeders_in_cluster_well_mixed(linkage_matrix, trafo_data_df, majority_feeder, cluster_idx):
    ...
    left_cluster_idx = int(linkage_matrix[cluster_idx - len(trafo_data_df.columns), 0])  # Subtract the df to get Z row idx from cluster idx
...
def _minority_feeder_subcluster(majority_feeder, initial_wrong_feeder_labels, sm_phases_in_cluster):
    ...
    all_min_phases_wrong_feeder = all(sm_phase in initial_wrong_feeder_labels for sm_phase in sm_phases_of_minority_feeder)

    # If not all minority feeder sm phases are previously declared a wrong feeder label, we assume a sub-cluster
    if not all_min_phases_wrong_feeder:
        has_minority_feeder_subcluster = True
    else:
...
def _check_for_feeder_phase_twists(trafo_data_df, trafo_id, trafo_phase_clusters, twisted_sm_phases, wrong_trafo_label):
    ...
    for feeder_phase in trafo_phase_clusters:
        filtered_phases = [p for p in trafo_phase_clusters[feeder_phase] if
                            p not in twisted_sm_phases.keys() and p not in wrong_trafo_label]
    ...
    for feeder_id in feeder_IDs:
        # if batch == 'first_batch':
            # NOTE: putting l1, l2, l3 to lovercase makes it compatible with only second batch, should be changed to fit for both batches
            # Extract a list of L1, L2 and L3 sm phases for the current feeder
        feeder_phases = {f"{l1}_phases_of_feeder": [p for p in sm_phases_filtered if (p.endswith(feeder_id)) and (l1 in p)],
        ...
        for f_p in [l1, l2, l3]:
            for t_p in [l1, l2, l3]:
                d[f"all_{f_p}_in_{t_p}"] = len([col for col in trafo_phase_clusters_wo_wrong_labels[t_p] if (col.endswith(feeder_id)) and (f_p in col)]) == len(feeder_phases[f"{f_p}_phases_of_feeder"])

        # Check for feeder to trafo phase patterns
        phase_patterns = {((l1, l1), (l2, l2), (l3, l3)): sum([d[f"all_l1_in_l1"], d[f"all_l2_in_l2"], d[f"all_l3_in_l3"]]) >= 2,
                        ((l1, l3), (l3, l1), (l2, l2)): d[f"all_l3_in_l1"] and d[f"all_l1_in_l3"] and d[f"all_l2_in_l2"],
                        ((l1, l2), (l2, l1), (l3, l3)): d[f"all_l2_in_l1"] and d[f"all_l1_in_l2"] and d[f"all_l3_in_l3"],
                        ((l2, l3), (l3, l2), (l1, l1)): d[f"all_l3_in_l2"] and d[f"all_l2_in_l3"] and d[f"all_l1_in_l1"],
                        ((l1, l2), (l2, l3), (l3, l1)): d[f"all_l2_in_l1"] and d[f"all_l3_in_l2"] and d[f"all_l1_in_l3"],
                        ((l1, l3), (l2, l1), (l3, l2)): d[f"all_l3_in_l1"] and d[f"all_l1_in_l2"] and d[f"all_l2_in_l3"]
                        }
    ...
    for twisted_sm_phase in twisted_sm_phases:
        ...
        trafo_phases = list({k: v for k, v in sm_to_trafo_phase_mapping.items() if k not in twisted_sm_phases.keys() and k not in wrong_trafo_label and actual_sm_phase in k and k.endswith(feeder_id)}.values())
        ...
        if len(trafo_phases) == 0:
            logging.info(f"[Trafo {trafo_id}] Feeder-phase unidentifiable for {twisted_sm_phase}. Falling back to SM phase ({actual_sm_phase}).")
...
def _create_wrong_cabinet_dict(linkage_matrix, wrong_feeder_labels, trafo_data_df, wrong_cabinet_labels):
    ...
    def get_column_names_in_cluster(clusters_containing_column, wrong_feeder_labels):
        ...
        for cluster in clusters_containing_column:
            ...
            cluster_column_names = [col for col in cluster if col not in wrong_feeder_labels.keys()]
    ...
    for wrong_feeder_col in wrong_feeder_labels:
        clusters_containing_column = find_clusters_containing_column(linkage_matrix, wrong_feeder_col, trafo_data_df.columns)
    ...
    for wrong_feeder_col in wrong_feeder_labels.keys():
...
def _rule_based_clustering_evaluation(trafo_id, trafo_data_df, linkage_matrix, initial_wrong_feeder_labels):
    ...
    def recursive_cluster_evaluation(trafo_id, cluster_idx, sm_phases_in_parent_cluster=[]):
        ...
        wrong_trafo_label_current_cluster = _check_for_wrong_trafo_sm(trafo_data_df,
                                                                                sm_phases_in_cluster,
                                                                                SMs_in_cluster,
                                                                                feeder_ids_in_parent_cluster)
        ...
        # Check if current cluster has minority feeder sub-cluster (i.e. not all minorities declared wrong feeder label in pre-evaluation)
        if num_of_feeders_in_cluster > 1 and majority_feeder and initial_wrong_feeder_labels is not None:
            has_minority_feeder_subcluster = _minority_feeder_subcluster(majority_feeder, initial_wrong_feeder_labels, sm_phases_in_cluster)
        else:
            has_minority_feeder_subcluster = False

        # Check if current cluster is a feeder phase cluster and evaluate it if so. Otherwise, continue with sub-clusters
        if majority_feeder is not None and majority_phase is not None and is_well_mixed and not has_minority_feeder_subcluster:

            # Create a key for the cluster
            feeder_phase_cluster_key = _create_feeder_phase_cluster_key(majority_feeder, majority_phase, feeder_phase_clusters)
            ...
            wrong_phase_label_current_cluster = _check_for_twisted_sm_phases(sm_phases_in_cluster, majority_phase, backup_phase_reference)
            ...
            wrong_feeder_label_current_cluster = _check_for_wrong_feeder_labels(sm_phases_in_cluster, majority_feeder, feeder_phase_cluster_key)
            ...
            doubled_phase_connection_current_cluster = _check_for_double_phase_connections(sm_phases_in_cluster, SMs_in_cluster)
    ...
    # Check for feeder phase twists and return dict which indicates for each SM phase to which trafo phase it is connected
    sm_to_trafo_phase_mapping, twisted_feeder_phases = _check_for_feeder_phase_twists(trafo_data_df, trafo_id, trafo_phase_clusters, twisted_sm_phases, wrong_trafo_labels)

    # Remove wrong trafo phases from sm phase twists
    twisted_sm_phases = {sm_phase: feeder_phase for sm_phase, feeder_phase in twisted_sm_phases.items() if sm_phase not in wrong_trafo_labels}

    # Add wrong cabinet info only for wrong feeder IDs
    wrong_cabinet_labels = _create_wrong_cabinet_dict(linkage_matrix, wrong_feeder_labels, trafo_data_df, wrong_cabinet_labels)

    # Put results into a results dict
    cluster_evaluation_results = {"feeder_phase_clusters": feeder_phase_clusters,
...
def _check_wrong_feeder_label_consistency(wrong_feeder_labels, corrupted_phases):
    ...
    for feeder_id in wrong_feeder_ids:
        ...
        wrong_feeder_labels_current_feeder = {key: value for key, value in wrong_feeder_labels.items() if feeder_id in value}

        # Create a list of SMs with wrong feeder labels
        sm_ids_with_wrong_feeder_label = [sm_phase.split('_')[2] for sm_phase in wrong_feeder_labels_current_feeder.keys()]
    ...
    # Check if in case of a potential wrong feeder label, the same wrong label appears on all uncorrupted phases of an SM
    verified_wrong_feeder_labels = []

    for sm_phase in wrong_feeder_labels.keys():

        sm_id = sm_phase.split("_")[2]

        # Get the number of uncorrupted phases of the current SM
        num_uncorrupted_phases = 3 - corrupted_phases_count_per_SM[sm_id] if sm_id in corrupted_phases_count_per_SM else 3

        # If all available (uncorrupted) phases of an SM have the same wrong feeder label, add to list
        if (sm_id in phases_with_wrong_feeder_label_counter) and (phases_with_wrong_feeder_label_counter[sm_id] == num_uncorrupted_phases):
...
def populate_result_df(trafo_results, cluster_evaluation, corrupted_phases, unconnected_phases):
    ...
    def _add_to_results(sm_list, result_col, alt_list=None, fixed_res=None, other_phase_info=None):
        ...
        for sm in sm_list:
            ...
            sm_mask = ((trafo_results["Feeder ID"] == feeder_id) &
            ...
            if alt_list:
                ...
                for phase in [f'{phase_l}1', f'{phase_l}2', f'{phase_l}3']:
                    sm_mask_other_phase = ((trafo_results["Feeder ID"] == feeder_id) &
                                            (trafo_results["Cabinet ID"] == cabinet_id) &
                                            (trafo_results["SM ID"] == sm_id) &
    ...
    _add_to_results(corrupted_phases, ['Feeder Phase', 'Trafo Phase', 'True Feeder ID', 'True Trafo ID'], fixed_res="nan")

    # Include unconnected phase information
    _add_to_results(unconnected_phases,['Feeder Phase', 'Trafo Phase', 'True Feeder ID', 'True Trafo ID'], fixed_res="nan")
    ...
    for index, sm_phase in trafo_results.iterrows():
        ...
        sm_phase_label = f"voltage_{sm_phase['SM Phase']}_{sm_phase['SM ID']}_{sm_phase['Cabinet ID']}_{sm_phase['Feeder ID']}"
        ...
        if sm_phase_label in cluster_evaluation["twisted_sm_phases"].keys():
        ...
        if sm_phase_label in cluster_evaluation["wrong_feeder_labels"].keys():
...
def _initialize_trafo_result_df(trafo_data_df_cols, trafo_id):
    ...
    for entry in trafo_data_df_cols:
        ...
        sm_phase_rows.append([trafo_id, feeder_id, cabinet_id, sm_id, sm_phase, sm_phase, "nan", feeder_id, trafo_id, None, "nan"])
    result_df_cols = ['Trafo ID', 'Feeder ID', 'Cabinet ID', 'SM ID', 'SM Phase', 'Feeder Phase', 'Trafo Phase',
                        'True Feeder ID', 'True Trafo ID', 'Likely Cabinet ID', 'Status']
...
def _save_result_plot(trafo_id, trafo_data_df, linkage_matrix, cluster_evaluation, s3_connector, cfg):
    ...
    feeder_phase_clusters_inverted = {sm_phase: feeder_phase for feeder_phase, phases in feeder_phase_clusters.items() for sm_phase in phases}
    ...
    for i, i12 in enumerate(linkage_matrix[:, :2].astype(int)):
        c1, c2 = (link_cols[x] if x > len(linkage_matrix) else feeder_phase_clusters_colors[feeder_phase_clusters_inverted[trafo_data_df.columns[x]]] for x in i12)
    ...
    for feeder_phase in twisted_feeder_phases:
        leaf_positions = [i for i, label in enumerate(dendro['ivl']) if
                            label.endswith(feeder_phase.split('_')[0]) and
                            feeder_phase.split('_')[1] in label]
    ...
    ax = plt.gca()
    ax.annotate("Phase   SM   Cabinet   Feeder",
                xy=(0, 0),
                xytext=(-10, -155),
    ...
    for sm_phase_label_obj in ax.get_xmajorticklabels():
        ...
        if (sm_phase in twisted_sm_phases) and (sm_phase in wrong_feeder_labels):
            ...
            sm_phase_label_obj.set_bbox(dict(facecolor='none', edgecolor='red', linewidth=1, boxstyle='round,pad=0.0001'))
    ...
    s3_path = f"{cfg['results_dir']}/{filename}"
    s3_connector.save_plot(s3_path, fig, format="svg", overwrite=True)
    plt.close(fig)

# TODO: add this to the dag
# @staticmethod
# def _extract_list_of_unconnected_sm_phases(sm_clf_result, batch):

#     unconnected_phases = []

#     for sm_id in sm_clf_result:

#         connected_phases = sm_clf_result[sm_id]["Connectivity"]["Connected Phases"]

#         if isinstance(connected_phases, list) and connected_phases:

#             unconnected_phases_of_sm = [phase for phase in ["L1", "L2", "L3"] if phase not in connected_phases]

#             for phase in unconnected_phases_of_sm:

#                 if batch != "first_batch":
#                     phase = phase.lower()

#                 unconnected_phases.append(f"voltage_{phase}_{sm_id}_{sm_clf_result[sm_id]['Topology']['Cabinet ID']}_{sm_clf_result[sm_id]['Topology']['Feeder ID']}")  # TODO is this relevant to change for loading data?

#     return unconnected_phases
...
def identify_sm_topology(trafo_ids, cfg, SM_topology_mapping):
    ...
    # TODO: Move this into the internal method, generate a list of smart meter ids per trafo, and run or querry the SM classification for these lists of SMs per trafo.
    # Load SM characterization
    # logging.info("Loading smart meter classification results...")
    # if isinstance(self.sm_clf_file, dict):
    #     sm_clf_result = self.sm_clf_file
    # else:
    #     sm_clf_path = os.path.join(self.ROOT_DIR,  "nils_sm_classification", "Results", self.sm_clf_file, f'{self.sm_clf_file}_SM_characterization.json')

    #     # Check if SM classification dict exists
    #     if os.path.isfile(sm_clf_path):
    #         with open(os.path.join(sm_clf_path), 'r') as f:
    #             logging.info(f"Returning existing smart meter classification results from {sm_clf_path}...")
    #             sm_clf_result = json.load(f)
    #     else:
            
    #         logging.info(f"Smart meter classification results not found at {sm_clf_path}. Running smart meter classification...")
    #         # Load SM Classifier if not already done
    #         if not self.SMClassifier:
    #             from nils_sm_classification.sm_classifier import SMClassifier
    #             self.SMClassifier = SMClassifier(self.smc_cfg, self.DataExtractor)

    #         # Get list of SM IDs below the given transformer IDs
    #         sm_ids = self.SM_topology_mapping.loc[self.SM_topology_mapping["Transformer ID"].isin(self.trafo_ids),"Meter ID"].tolist()
    #         sm_clf_result, _ = self.SMClassifier.classify_smart_meters(sm_ids=sm_ids, run_name=self.sm_clf_file)

    # Extract list of unconnected SM phases from the SM classification results
    # unconnected_phases = self._extract_list_of_unconnected_sm_phases(sm_clf_result, self.batch)
    ...
    for trafo_id in tqdm(trafo_ids):
        try:
            ...
            # Get unconnected phases from SM classification results
            unconnected_phases = []

            # TODO: This get_meters_from_transformer does currently not work
            # sm_ids_from_trafo = topology_controller.get_meters_for_transformer(int(trafo_id))

            # Get from SM_topology_mapping instead
            sm_ids_from_trafo = SM_topology_mapping.loc[SM_topology_mapping["Transformer ID"] == int(trafo_id), "Meter ID"].tolist()
            # sm_ids_from_trafo = [str(sm_id) for sm_id in sm_ids_from_trafo]
            logging.info(f"SMs below trafo: {sm_ids_from_trafo}")

            for sm_id in sm_ids_from_trafo:

                sm_topology_info = meta_controller.get_sm_characterization(sm_id)
                
                # TODO: add this to DAG
                ...
                #         app.run()
                #     sm_topology_info = meta_controller.get_sm_characterization(sm_id)  # re-fetch

                connectivity = sm_topology_info.get("Connectivity")
                if connectivity and "Connected Phases" in connectivity:
                    connected_phases = connectivity["Connected Phases"]
                    unconnected_phases_of_sm = [phase for phase in ["L1", "L2", "L3"] if phase not in connected_phases]

                    for phase in unconnected_phases_of_sm:
                        unconnected_phases.append(f"voltage_{phase}_{sm_id}_{sm_topology_info['Topology']['Cabinet ID']}_{sm_topology_info['Topology']['Feeder ID']}")
            
            
            # Load raw sm phase voltage time series of all SMs below current transformer trafo_id
            # trafo_data_df = _load_data_of_single_trafo(trafo_id=trafo_id, save=self.save_missing_trafo_datasets)
            # TODO: add the topology processs level (doesnt work now)
            logging.info(sm_ids_from_trafo)
            ...
            # correct trafo_data_df so the columns, each column have the voltage_{phase}_{sm_id}
            # Right now it has ['meter_number', 'timestamp', 'voltage_l1', 'voltage_l2', 'voltage_l3', 'active_power_p14_l1', 'active_power_p14_l2', 'active_power_p14_l3', 'active_power_p23_l1', 'active_power_p23_l2', 'active_power_p23_l3', 'reactive_power_q12_l1', 'reactive_power_q12_l2', 'reactive_power_q12_l3', 'reactive_power_q34_l1', 'reactive_power_q34_l2', 'reactive_power_q34_l3', 'thdu_l1', 'thdu_l2', 'thdu_l3', 'thdi_l1', 'thdi_l2', 'thdi_l3', 'dt', 'shard']
            voltage_cols = ["voltage_l1", "voltage_l2", "voltage_l3"]
            trafo_data_df = trafo_data_df[["timestamp", "meter_number"] + voltage_cols]
            trafo_data_df = trafo_data_df.pivot(index="timestamp", columns="meter_number", values=voltage_cols,)
            ...
            trafo_data_df, corrupted_phases = _clean_data_of_single_unit(trafo_data_df=trafo_data_df,
            ...
            pre_cluster_evaluation = _rule_based_clustering_evaluation(trafo_id, trafo_data_df, linkage_matrix, initial_wrong_feeder_labels=None)

            # Check the wrong feeder labels for pattern match and redo cluster evaluation with this information
            initial_wrong_feeder_labels = _check_wrong_feeder_label_consistency(pre_cluster_evaluation["wrong_feeder_labels"], corrupted_phases+unconnected_phases)

            # Evaluate clusters
            cluster_evaluation = _rule_based_clustering_evaluation(trafo_id, trafo_data_df, linkage_matrix, initial_wrong_feeder_labels)

            # Save dendrogram with feeder clusters and SM topology identification results
            if cfg["save_plots"]:
                _save_result_plot(trafo_id, trafo_data_df, linkage_matrix, cluster_evaluation, s3_connector, cfg)
            ...
            print(trafo_results)
            print("the type, length, and columns of the result df")
            print(type(trafo_results))
            print(len(trafo_results))
            print(trafo_results.columns)
            ...
            logging.error(f"Phase mapping failed for transformer {trafo_id} with error: {str(e)}. Continuing with next transformer.")
            continue
